...
{
    // ponytail: 밤 칸은 확정 사양이 아니다. 앞의 7종은 기획서 9.1에서 취소선 처리된
    // 밤 패시브 후보이고, 여덟 번째(제빵사)의 `메아리`만 9.2의 밤 액티브 표에서 가져왔다
    // — 9.1 후보가 7종뿐이라 8종을 채울 수 없었다. 즉 두 출처가 섞여 있다.
    // 목업 2번이 아직 `NIGHT PASSIVE`로 그려져 있어 화면을 눈으로 대조할 수 있게 채운
    // 값이며, 9.1이 확정되면 이 두 칸을 통째로 갈아 끼운다.
