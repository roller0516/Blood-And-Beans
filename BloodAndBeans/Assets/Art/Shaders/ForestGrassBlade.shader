// 원본: Youssef-Afella/UnityURP-InfiniteGrass (MIT)의 GrassBladeShader.
// 프로젝트 소유 복사본이다 — 서드파티 원본은 고치지 않는다 (AGENTS.md).
// 원본과 다른 점은 하나뿐이다: 밟기(_GrassTramplers)로 잎 끝을 눕히고 키를 눌러 준다.
Shader "BloodAndBeans/ForestGrassBlade"
{
    ...
    Properties
    {
        ...
        _GrassTrampleRadius("Trample Radius", Float) = 1.2
        _GrassTrampleStrength("Trample Strength", Float) = 0.6
        _GrassTrampleSquash("Trample Squash", Range(0, 1)) = 0.3
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float _GrassTrampleRadius;
                float _GrassTrampleStrength;
                float _GrassTrampleSquash;
            ...
            // 밟는 주체(플레이어)의 자리. xy=월드 XZ, w=1이면 쓰는 칸이다.
            // `GrassTrample`이 매 프레임 넣는다. 재질이 아니라 전역이라 CBUFFER 밖에 둔다.
            #define GRASS_TRAMPLERS 4
            float4 _GrassTramplers[GRASS_TRAMPLERS];
            ...
            Varyings vert(Attributes IN, uint instanceID : SV_InstanceID)
            {
                ...
                //Trample: 밑동은 그대로 두고 잎 끝만 밟는 쪽의 반대 방향으로 눕힌다.
                //뿌리까지 옮기면 풀이 밀려나는 게 아니라 뽑혀서 미끄러지는 것처럼 보인다.
                float2 trample = 0;
                float squash = 0;
                [unroll]
                for (int t = 0; t < GRASS_TRAMPLERS; t++)
                {
                    float2 away = pivot.xz - _GrassTramplers[t].xy;
                    float falloff = _GrassTramplers[t].w
                                  * saturate(1 - length(away) / max(_GrassTrampleRadius, 0.001));
                    falloff *= falloff;                       //가장자리를 부드럽게
                    trample += normalize(away + float2(1e-4, 1e-4)) * falloff;
                    squash = max(squash, falloff);
                }
                bladeDirection.xz += trample * _GrassTrampleStrength * IN.positionOS.y;
                grassHeight *= 1 - squash * _GrassTrampleSquash;
